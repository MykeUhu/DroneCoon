...
#include "Game/GameMode/UhuGameMode.h"
#include "Game/Log/Log.h"

/**
 * @class AUhuGameMode
 * @brief Diese Klasse erweitert den Standard-GameMode, um GameLift-Funktionalitäten
 *        und serverseitige Parameterverarbeitung hinzuzufügen.
 */
AUhuGameMode::AUhuGameMode()
{
}

/**
 * @brief Wird jeden Tick aufgerufen und führt serverseitige Logik aus.
 * 
 * @param DeltaTime Zeit seit dem letzten Tick.
 */
void AUhuGameMode::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);
}

/**
 * @brief Initialisiert den Spielmodus und startet GameLift-Integration (falls aktiviert).
 */
void AUhuGameMode::BeginPlay()
{
	Super::BeginPlay();
#if WITH_GAMELIFT
	InitGameLift();
 ...
 */
void AUhuGameMode::InitGameLift()
{
    UE_LOG(UhuLog, Log, TEXT("Initializing the GameLift Server"));
    ...
    auto OnGameSession = [=](Aws::GameLift::Server::Model::GameSession gameSession)
    {
        ...
        UE_LOG(UhuLog, Log, TEXT("GameSession Initializing: %s"), *GameSessionId);
    ...
    {
        UE_LOG(UhuLog, Log, TEXT("Game Server process is terminating."));
    ...
    auto OnHealthCheck = []()
    {
        UE_LOG(UhuLog, Log, TEXT("Performing Health Check"));
        return true;
    };
    ...
    // Logge die ProcessId
    UE_LOG(UhuLog, Log, TEXT("PID: %s"), *ServerParameters.m_processId);
    ...
    UE_LOG(UhuLog, Log, TEXT("Calling Process Ready"));
 ...
 */
void AUhuGameMode::SetServerParameters(FServerParameters& OutServerParameters)
{
    ...
    {
        UE_LOG(UhuLog, Log, TEXT("AUTH_TOKEN: %s"), *OutServerParameters.m_authToken);
    }
    ...
    {
        UE_LOG(UhuLog, Log, TEXT("HOST_ID: %s"), *OutServerParameters.m_hostId);
    }
    ...
    {
        UE_LOG(UhuLog, Log, TEXT("FLEET_ID: %s"), *OutServerParameters.m_fleetId);
    }
    ...
    {
        UE_LOG(UhuLog, Log, TEXT("WEBSOCKET_URL: %s"), *OutServerParameters.m_webSocketUrl);
    }
    ...
    {
        UE_LOG(UhuLog, Log, TEXT("AWS_REGION: %s"), *OutServerParameters.m_awsRegion);
    }
    ...
    {
        UE_LOG(UhuLog, Log, TEXT("ACCESS_KEY: %s"), *OutServerParameters.m_accessKey);
    }
    ...
    {
        UE_LOG(UhuLog, Log, TEXT("SECRET_KEY: %s"), *OutServerParameters.m_secretKey);
    }
    ...
    {
        UE_LOG(UhuLog, Log, TEXT("SESSION_TOKEN: %s"), *OutServerParameters.m_sessionToken);
    }
    
    // The PID of the running process
    OutServerParameters.m_processId = FString::Printf(TEXT("%d"), GetCurrentProcessId());
    UE_LOG(UhuLog, Log, TEXT("PID: %s"), *OutServerParameters.m_processId);
}
 ...
 */
void AUhuGameMode::ParseCommandLinePort(int32& OutPort)
{
